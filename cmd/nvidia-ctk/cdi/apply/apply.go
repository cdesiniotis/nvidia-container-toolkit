...
// defaultDeviceKind is the default CDI kind (vendor/class) used
// when constructing a fully qualified CDI device name
const defaultDeviceKind = "nvidia.com/gpu"
...
func (m command) build() *cli.Command {
	...
	return &cli.Command{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:        "mode",
				Usage:       "Mode for applying the CDI spec to different inputs [oci | fstab | hook].",
				Value:       "oci",
			...
			&cli.StringSliceFlag{
				...
				Usage:       "Specify the CDI device names to apply. Device names should be in the format vendor/class=name (e.g., nvidia.com/gpu=0). If a device name is not fully qualified, it is qualified using the value of --device-kind (e.g., 0 becomes nvidia.com/gpu=0)",
				Destination: &opts.devices,
				Sources:     cli.EnvVars("NVIDIA_CTK_CDI_APPLY_DEVICES"),
			},
			&cli.StringFlag{
				Name:        "device-kind",
				Usage:       "Specify the CDI device kind (vendor/class) used to construct fully-qualified CDI device names. Only used if devices specified in --devices are not fully-qualified.",
				Value:       defaultDeviceKind,
				Destination: &opts.deviceKind,
				Sources:     cli.EnvVars("NVIDIA_CTK_CDI_APPLY_DEVICE_KIND"),
			},
			&cli.StringSliceFlag{
				Name:        "spec-dir",
			...
			&cli.StringFlag{
				Name:        "container-root",
				Usage:       "Specify the path to the container root filesystem. If set, a --container-root flag is appended to each hook command generated in 'hook' mode. (Only used in 'hook' mode)",
				Destination: &opts.containerRoot,
				Sources:     cli.EnvVars("NVIDIA_CTK_CDI_APPLY_CONTAINER_ROOT"),
			},
		},
	}
}
...
func (m command) validateFlags(opts *options) error {
	...
	vendor, class := parser.ParseQualifier(opts.deviceKind)
	if err := parser.ValidateVendorName(vendor); err != nil {
		return fmt.Errorf("invalid device kind %q: %w", opts.deviceKind, err)
	}
...
// qualifyDevices ensures that all requested device names are fully qualified.
// Any device name that is not already in the vendor/class=name format is
// qualified using the configured device kind (e.g. 0 becomes nvidia.com/gpu=0).
func (m command) qualifyDevices(opts *options) []string {
	vendor, class := parser.ParseQualifier(opts.deviceKind)

	devices := make([]string, 0, len(opts.devices))
	for _, device := range opts.devices {
		if !parser.IsQualifiedName(device) {
			qualified := parser.QualifiedName(vendor, class, device)
			m.logger.Debugf("Qualified device %q as %q", device, qualified)
			device = qualified
		}
...
func (m command) run(ctx context.Context, cmd *cli.Command, opts *options) error {
	m.logger.Debugf("apply command invoked with mode: %s", opts.mode)

	// If no devices specified, return early
	if len(opts.devices) == 0 {
		m.logger.Debugf("No devices specified")
		return nil
	}

	// Ensure that all requested device names are fully qualified CDI device names.
	opts.devices = m.qualifyDevices(opts)

	m.logger.Debugf("Processing %d CDI device(s)", len(opts.devices))
	...
	case "hook":
		if err := m.applyHookMode(ctx, opts, registry, outputWriter); err != nil {
			return fmt.Errorf("failed to apply hook mode: %v", err)
		}
	default:
...
func (m command) applyFstabMode(ctx context.Context, opts *options, registry *cdi.Cache, outputWriter io.Writer) error {
	...
	if len(unresolved) > 0 {
		return fmt.Errorf("unknown devices requested: %v", unresolved)
	}
...
func (m command) applyOCIMode(ctx context.Context, opts *options, registry *cdi.Cache, outputWriter io.Writer) error {
	...
	m.logger.Debugf("Applying CDI devices to OCI spec: %v", opts.devices)
